...
export class ProxyManager {
  ...
  getBestProxy(excludeIds: string[] = []): Proxy | null {
    ...
    const activeProxies = this.proxies.filter((p) => !p.isRetired && !excludeIds.includes(p.id));
  ...
  getProxyForSession(sessionId: string): Proxy | null {
    ...
    if (bestProxy) {
      ...
      this._log(
        `Binding session ${sessionId} → proxy ${bestProxy.id} (success rate: ${((bestProxy.successRate || 1) * 100).toFixed(1)}%)`,
      );
      return bestProxy;
    }
  ...
  switchProxyForSession(sessionId: string, reason: string = 'error'): Proxy | null {
    ...
    if (newProxy) {
      ...
      this._log(
        `Switched session ${sessionId} from ${currentProxyId || 'none'} → ${newProxy.id} (reason: ${reason})`,
      );
      return newProxy;
    }
    ...
    for (const proxy of this.proxies) {
      if (proxy.isRetired && proxy.retiredAt) {
        ...
        if (cooledTime >= this.cooldownPeriodMs) {
          ...
          revivedCount++;
          this._log(
            `Proxy ${proxy.id} revived after ${Math.round(cooledTime / 60000)} min cooldown`,
            'info',
          );
        }
      }
  ...
  markProxyFailed(proxyId: string, reason: string = 'unknown error'): void {
    ...
    if (proxy) {
      ...
      this._log(
        `Proxy ${proxyId} failed: ${reason} (errors: ${proxy.errorCount}, rate: ${(proxy.successRate * 100).toFixed(1)}%)`,
        'warn',
      );

      if (
        proxy.errorCount >= this.maxErrorCount ||
        proxy.consecutiveFailures >= this.maxConsecutiveFailures
      ) {
  ...
  private retireProxy(proxyId: string): void {
    ...
    if (proxy) {
      ...
      this._log(
        `Proxy ${proxyId} RETIRED (will revive after ${this.cooldownPeriodMs / 60000} min cooldown)`,
        'warn',
      );
    }
  }

...
export class ProgressManager {
    ...
    public async loadProgress(targetType: string, targetValue: string, jobId?: string): Promise<ScrapingProgress | null> {
        ...
        try {
            ...
            if (progress.jobId) {
                ...
                if (progress.lastCursor) {
                    await CheckpointRepository.saveCheckpoint(progress.jobId, 'timeline_cursor', progress.lastCursor);
                }
                if (progress.lastTweetId) {
                    await CheckpointRepository.saveCheckpoint(progress.jobId, 'last_tweet_id', progress.lastTweetId);
                }
            }

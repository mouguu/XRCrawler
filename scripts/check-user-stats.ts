      ...
      if (stats) {
        ...
        if (stats.statuses_count) {
          ...
          if (difference <= 50) {
            ...
            console.log(
              `     Collected 827 which suggests the API stopped early (normal behavior)`,
            );
          } else {
        ...
        const timelineInstructions =
          userResponse?.data?.user?.result?.timeline?.timeline?.instructions ||
        ...
        if (timelineInstructions) {
          ...
          console.log(
            ...
            JSON.stringify(userResponse, null, 2).substring(0, 800),
          );
        }
      }
    }

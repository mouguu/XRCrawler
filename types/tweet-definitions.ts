 ...
 * 从 API 响应解析推文
 */
export function parseTweetFromApiResult(
  result: TweetResult,
...
export function parseTweetFromRestStatus(status: any, fallbackUsername?: string): Tweet | null {
  ...
  const text = status.full_text || status.extended_tweet?.full_text || status.text || '';
  ...
  return {
    ...
    hasMedia: Boolean(status.extended_entities?.media?.length || status.entities?.media?.length),
    username,
  ...
  for (const instruction of instructions) {
    ...
    for (const entry of instruction.entries) {
      ...
      if (entryId.startsWith('cursor-bottom')) {
        const cursorValue =
          entry.content?.value || entry.content?.cursorType || entry.content?.itemContent?.value;
        if (cursorValue) {
          return cursorValue;
        }
      }
    ...
    if (instruction.type === 'TimelineAddToModule') {
      ...
      for (const item of moduleItems) {
        if (item.entryId?.includes('cursor')) {
          const cursorValue = item.item?.itemContent?.value || item.item?.itemContent?.cursorType;
          if (cursorValue) {
            return cursorValue;
          }
        }

...
import { HandleRateLimit, RetryOnNetworkError } from '../../utils/decorators';
...
describe('Decorators', () => {
  describe('RetryOnNetworkError', () => {
    ...
    test('should retry on network errors', async () => {
      ...
      await expect(instance.networkErrorMethod()).rejects.toThrow('network timeout');
    ...
    test('should not retry non-network errors', async () => {
      ...
      await expect(instance.failingMethod('validation failed')).rejects.toThrow(
        'validation failed',
      );
  ...
  describe('HandleRateLimit', () => {
    ...
    test('should handle rate limit errors', async () => {
      ...
      await expect(instance.rateLimitedMethod()).rejects.toThrow(); // ScraperErrors.rateLimitExceeded() is thrown
    });
    ...
    test('should not catch non-rate-limit errors', async () => {
      ...
      await expect(instance.normalErrorMethod()).rejects.toThrow('Normal error');
    });
  });
});
